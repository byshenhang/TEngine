			...
			if (_textMeshPro)
			{
				...
				for (int i = 0; i < materialCount; i++)
				{
					...
					if (meshInfo.vertexCount > 0)
					{
						...
						// NOTE: For the vertexCount we don't use meshInfo.mesh.vertexCount because sometimes TMP has extra dummy (null) vertices in the mesh - meshInfo.vertexCount is accurate.
						_screenRect.AddVertexBounds(_renderSpace == FilterRenderSpace.Canvas ? null : this.transform, meshInfo.mesh.vertices, meshInfo.vertexCount);
					}
				}
			}
