...
namespace ChocDino.UIFX
{
	...
	public class BlurZoomFilter : FilterBase
	{
		...
		class BlurShader
		{
			...
			static class Prop
			{
				public static int CenterInvScale = Shader.PropertyToID("_CenterInvScale");
			...
			internal RenderTexture Render(RenderTexture sourceTexture, float scale, Vector2 center, bool weightsLinear, float dither, BlurDirectionalSide side)
			{
		...
		protected override RenderTexture RenderFilters(RenderTexture source)
		{
			if (GetAlpha() > 0f)
			{
				return _blurShader.Render(source, Mathf.Lerp(1f, _scale, _strength), (_center + Vector2.one) * 0.5f, _weights == BlurDirectionalWeighting.Linear, _dither * _strength * 0.1f, _side);
			}
			return null;
		}
